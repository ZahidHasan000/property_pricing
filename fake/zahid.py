...
# # CORS(app, resources={r"/predict": {"origins": "http://localhost:3003"}})
# CORS(app, origins='http://localhost:3003', methods=['GET', 'POST', 'OPTIONS'], allow_headers='Content-Type')


# # Load the property data from MongoDB
# property_data = list(mongo.db.property_data.find())
...
#     print('Predicted Sentiment polarity:', TextBlob(description).sentiment.polarity)
#     print('-' * 60)

# # Calculate sentiment polarity for all test reviews
# sentiment_polarity = [TextBlob(review).sentiment.polarity for review in test_reviews]
...
# numeric_features = ["number_of_bedrooms", "base_price", "bathrooms", "beds", "guests"]
# numeric_df = property_df[numeric_features].fillna(property_df[numeric_features].mean())

# categorical_features = ["location", "property_type", "option", "amenities", "seasonality", "bed_type", "neighborhood", "guest_type"]
# categorical_df = property_df[categorical_features].fillna(property_df[categorical_features].mode().iloc[0])
...
#     print("Description for sentiment analysis:", description)
...
#     print("Features for prediction:", X_test_scaled)
...
#     print("Location for nearest neighbor search:", latitude, longitude)
#     location_data = np.array([[latitude, longitude]])
#     neighbors_indices = nn_model.kneighbors(location_data, n_neighbors=k, return_distance=False)
#     # avg_neighbor_price = property_df.loc[neighbors_indices[0], "base_price"].mean()
#     avg_neighbor_price = round(property_df.loc[neighbors_indices[0], "base_price"].mean(), 2)
...
# 		    # save details
#             id =  mongo.db.property_data.insert_one({"location": location, "latitude": latitude, "longitude": longitude, "property_type": property_type, "option": option, "guests": guests, "description": description, "title":title, "base_price": base_price, "number_of_bedrooms": number_of_bedrooms, "amenities": cleaned_amenities, "seasonality": seasonality, "bathrooms": bathrooms, "bed_type": bed_type, "beds": beds, "neighbourhood": neighborhood, "guest_type": guest_type, "sentiment": predicted_price, "prdicted_price": suggested_price, "nearest_price": nearest_price})
